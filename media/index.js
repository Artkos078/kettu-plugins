...
function openNative(url){
  ...
  try{
    lastUrl=String(url);openedCount++;
...
function installHook(){
  try{
    ...
    if(!C||!C.prototype||typeof C.prototype.render!=='function'){hookStatus='common Video class missing';return false;}
    VideoClass=C;originalRender=C.prototype.render;
    C.prototype.render=function(){
      try{
        ...
        if(pstore.enabled!==false&&url){
          var self=this;
          if(!self.__nativeVideoOriginalOpen&&typeof self.handleOpenFullScreen==='function')self.__nativeVideoOriginalOpen=self.handleOpenFullScreen.bind(self);
    ...
    return true;
  }catch(e){hookStatus='hook error';try{console.log('[NativeVideoPlayer v2.1]',e);}catch(_){}return false;}
}
function uninstallHook(){try{if(VideoClass&&originalRender&&VideoClass.prototype.render!==originalRender)VideoClass.prototype.render=originalRender;}catch(e){}VideoClass=null;originalRender=null;}
function Settings(){
  ...
  return React.createElement(ScrollView,{style:{padding:16}},
    ...
    React.createElement(Text,{style:{color:'#777',marginTop:12}},'Discord\'s media viewer is not opened at the same time, preventing an overlapping viewer from blocking the native close button.'),
