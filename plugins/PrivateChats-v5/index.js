(function(){
  ...
  function addCurrentServer(){var c=context();if(!c.guildId){toast('Open a server first');return false;}if(state.hiddenGuildIds.indexOf(c.guildId)===-1)state.hiddenGuildIds.push(c.guildId);save();unlocked=false;refresh();toast('Protected '+((c.guild&&c.guild.name)||'current server'));return true;}
  function addCurrentDM(){var c=context();if(!c.channelId){toast('Open a DM first');return false;}if(c.guildId){toast('Open a DM, not a server channel');return false;}if(state.hiddenChannelIds.indexOf(c.channelId)===-1)state.hiddenChannelIds.push(c.channelId);save();unlocked=false;refresh();toast('Protected current DM');return true;}
  ...
  function SettingsComponent(){
    ...
    function row(id,type){return React.createElement(View,{key:type+id,style:{flexDirection:'row',justifyContent:'space-between',paddingVertical:10}},React.createElement(Text,{style:{color:'white',flex:1}},id),React.createElement(Pressable,{onPress:function(){if(type==='g')state.hiddenGuildIds=state.hiddenGuildIds.filter(function(x){return String(x)!==String(id);});else state.hiddenChannelIds=state.hiddenChannelIds.filter(function(x){return String(x)!==String(id);});save();refresh();bump();}},React.createElement(Text,{style:{color:'#ff6b6b',fontWeight:'700'}},'Remove')));}
  ...
  function start(){unlocked=false;save();patchStores();toast('PrivateChats v5 loaded');}
