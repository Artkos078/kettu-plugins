(function () {
  ...
  var SERVER_CACHE_GUILD_NAME = "The Lounge";
  var persistentCache = V.channelMediaGalleryPersistent || null;
  ...
  if (!Array.isArray(storage.selectedChannelIds)) storage.selectedChannelIds = [];
  ...
  function readPinnedCache() {
    try {
      if (persistentCache && typeof persistentCache.load === "function") {
        ...
        if (saved && Array.isArray(saved.savedMedia) && isPinnedGuild(saved.savedGuildId || saved.selectedGuildId)) {
          saved.savedGuildId = SERVER_CACHE_GUILD_ID;
          saved.selectedGuildId = SERVER_CACHE_GUILD_ID;
          return saved;
        }
      }
    } catch (e) {}
  ...
  function writePinnedCache() {
    try {
      ...
      var guildId = String(storage.savedGuildId || "");
      if (!isPinnedGuild(guildId)) return;
      guildId = SERVER_CACHE_GUILD_ID;
  ...
  function clearPinnedCache() {
    try { if (persistentCache && typeof persistentCache.clear === "function") persistentCache.clear(); } catch (e) {}
  }
  ...
  function restorePinnedCache() {
    ...
    storage.selectedChannelId = storage.selectedChannelId || saved.selectedChannelId || saved.savedChannelId || null;
    storage.selectedGuildId = storage.selectedGuildId || saved.selectedGuildId || SERVER_CACHE_GUILD_ID;
    storage.lastChannelId = storage.lastChannelId || saved.lastChannelId || saved.savedChannelId || null;
    status.cache = true;
    return true;
  }
  ...
  function selectedChannelIds() {
    var ids = [];
    asArray(storage.selectedChannelIds).forEach(function (id) {
      id = String(id || "");
      if (id && ids.indexOf(id) === -1) ids.push(id);
    });
    if (!ids.length && storage.selectedChannelId) ids.push(String(storage.selectedChannelId));
    return ids;
  }

  function selectedChannelLabel() {
    var ids = selectedChannelIds();
    if (storage.forceLoadChannels && ids.length > 1) return ids.length + " channels selected";
    var id = ids[0] || (storage.forceLoadChannels ? null : storage.lastChannelId);
    var channel = getChannel(id);
    return readChannelName(channel) || id || "Choose a channel";
  }

  function setSelectedChannelIds(ids) {
    var clean = [];
    asArray(ids).forEach(function (id) {
      id = String(id || "");
      if (id && clean.indexOf(id) === -1) clean.push(id);
    });
    storage.selectedChannelIds = clean;
    storage.selectedChannelId = clean[0] || null;
  }

  function toggleSelectedChannel(id) {
    id = String(id || "");
    if (!id) return;
    var ids = selectedChannelIds();
    ...
    storage.lastChannelId = id;
    var channel = getChannel(id);
    if (channel && (channel.guild_id || channel.guildId)) storage.selectedGuildId = String(channel.guild_id || channel.guildId);
  }
  ...
  async function openChannelFast(channelId) {
    channelId = String(channelId || "");
    if (!channelId) return "no channel";
    var channel = getChannel(channelId) || remoteChannels[channelId] || null;
    var guildId = String((channel && (channel.guild_id || channel.guildId)) || storage.selectedGuildId || storage.savedGuildId || "@me");
    var route = "/channels/" + (guildId || "@me") + "/" + channelId;
    var attempts = [];
    ...
    var selector = findByProps("selectChannel") || findByProps("selectGuild", "selectChannel");
    if (selector && typeof selector.selectChannel === "function") {
      if (tryCall("selectChannel(guild,channel)", function () { return selector.selectChannel(guildId === "@me" ? null : guildId, channelId); })) { await wait(250); return "opened with selectChannel"; }
      if (tryCall("selectChannel(channel)", function () { return selector.selectChannel(channelId); })) { await wait(250); return "opened with selectChannel"; }
    }
    var router = findByProps("transitionTo") || findByProps("replaceWith", "transitionTo") || findByProps("push", "pop");
    if (router && typeof router.transitionTo === "function") {
      if (tryCall("transitionTo(route)", function () { return router.transitionTo(route); })) { await wait(250); return "opened with transitionTo"; }
    }
    if (router && typeof router.push === "function") {
      if (tryCall("push(route)", function () { return router.push(route); })) { await wait(250); return "opened with push"; }
    }
    var jumper = findByProps("jumpToMessage") || findByProps("jumpToMessageId");
    if (jumper && typeof jumper.jumpToMessage === "function") {
      if (tryCall("jumpToMessage", function () { return jumper.jumpToMessage({ channelId: channelId, guildId: guildId === "@me" ? null : guildId }); })) { await wait(250); return "opened with jumpToMessage"; }
    }
    var dispatcher = findByProps("dispatch", "subscribe");
    if (dispatcher && typeof dispatcher.dispatch === "function") {
      if (tryCall("dispatch CHANNEL_SELECT", function () { dispatcher.dispatch({ type: "CHANNEL_SELECT", channelId: channelId, guildId: guildId === "@me" ? null : guildId }); })) { await wait(250); return "selected with dispatcher"; }
    }
    return "open unavailable" + (attempts.length ? " (" + attempts.slice(0, 2).join("; ") + ")" : "");
  }
  ...
  async function loadMedia(channelId, force) {
    ...
    var result = await loadMediaFromChannels([channelId], force);
    result.channelId = channelId;
    return result;
  }

  async function loadMediaFromChannels(channelIds, force) {
    var ids = asArray(channelIds).map(function (id) { return String(id || ""); }).filter(Boolean);
    var unique = [];
    ids.forEach(function (id) { if (unique.indexOf(id) === -1) unique.push(id); });
    if (force && !unique.length) throw new Error("Choose at least one channel in the selected server first.");
    if (!unique.length) unique.push(rememberCurrentChannel());
    unique = unique.filter(Boolean);
    if (!unique.length) throw new Error("Pick a loaded channel, or open the channel in Discord once so the plugin can see it.");
    ...
    var combined = [];
    var scanGuildId = "";
    for (var i = 0; i < unique.length && combined.length < max; i++) {
      var channelId = unique[i];
      var openStatus = "not opened";
      if (force && storage.openBeforeForceLoad !== false) openStatus = await openChannelFast(channelId);
      var remote = await getRemoteMessages(channelId, limit);
      fetchedChannels[String(channelId)] = true;
      combined = dedupeSavedMedia(combined.concat(collectMedia(remote, max - combined.length))).slice(0, max);
      openStatuses.push(channelId + ": " + openStatus);
      storage.savedChannelId = channelId;
      var savedChannel = getChannel(channelId);
      var channelGuildId = savedChannel && String(savedChannel.guild_id || savedChannel.guildId || "");
      if (!scanGuildId && channelGuildId) scanGuildId = channelGuildId;
    }
    if (!scanGuildId && storage.selectedGuildId) scanGuildId = String(storage.selectedGuildId);
    storage.savedGuildId = scanGuildId || null;
    storage.savedMedia = dedupeSavedMedia(combined).slice(0, max);
    storage.savedAt = new Date().toISOString();
    writePinnedCache();
    status.cache = true;
    return { channelId: unique.join(", "), channelIds: unique, media: storage.savedMedia, source: unique.length + " channel" + (unique.length === 1 ? "" : "s"), openStatus: openStatuses.join(" | ") };
  }
  ...
  function Settings() {
    ...
    var hiddenState = React.useState(true), hidden = hiddenState[0], setHidden = hiddenState[1];
    ...
    var msgState = React.useState(storage.savedMedia.length ? (restoredAtStart ? SERVER_CACHE_GUILD_NAME + " cache restored and hidden. Choose Show Loaded Cache or Force." : "Saved media is hidden. Choose Show Loaded Cache or Force.") : "Choose Loaded or Force. Media stays hidden until you load it."), message = msgState[0], setMessage = msgState[1];
    ...
    var selectedIds = selectedChannelIds();
    var selectedName = selectedChannelLabel();
    ...
    function chooseGuild(id) { storage.selectedGuildId = String(id); setSelectedChannelIds([]); setGuildPickerOpen(false); setChannelSearch(""); setChannels(getChannelRows("")); bump(); setMessage("Selected server: " + (readGuildName(getGuild(id)) || String(id))); }
    function chooseChannel(id) {
      if (storage.forceLoadChannels) {
        toggleSelectedChannel(id);
        bump();
        setMessage("Selected " + selectedChannelIds().length + " channel(s).");
        return;
      }
      setSelectedChannelIds([id]);
      storage.lastChannelId = String(id);
      var channel = getChannel(id);
      if (channel && (channel.guild_id || channel.guildId)) storage.selectedGuildId = String(channel.guild_id || channel.guildId);
      setPickerOpen(false);
      bump();
      setMessage("Selected channel: " + String(id));
    }
    function clearChannelSelection() { setSelectedChannelIds([]); bump(); setMessage("Channel selection cleared."); }
    function chooseLoadMode(force) {
      storage.forceLoadChannels = !!force;
      setHidden(true);
      setItems([]);
      setGuilds(getGuildRows(guildSearch));
    ...
    function showLoadedCache() {
      setHidden(false);
      var visible = filtered(storage.savedMedia);
      setItems(visible);
      setMessage(visible.length ? "Showing " + visible.length + " saved media." : "No saved media yet. Run Loaded or Force first.");
    }
    function selectShownChannels(loadedOnly) {
      var rows = getChannelRows(channelSearch);
      if (loadedOnly) rows = rows.filter(function (row) { return row.source === "loaded"; });
      setSelectedChannelIds(rows.map(function (row) { return row.id; }));
      bump();
      setMessage("Selected " + rows.length + (loadedOnly ? " loaded" : " shown") + " channel(s).");
    }
    async function runLoad() {
      setLoading(true); setMessage("Scanning channel media...");
      try {
        var ids = storage.forceLoadChannels ? selectedChannelIds() : [storage.selectedChannelId || storage.lastChannelId || rememberCurrentChannel()];
        var result = await loadMediaFromChannels(ids, storage.forceLoadChannels);
        setHidden(false);
        setGuilds(getGuildRows(guildSearch)); setChannels(getChannelRows(channelSearch)); setItems(filtered(result.media)); setMessage("Saved " + result.media.length + " media from " + result.source + ". Open: " + (result.openStatus || "not opened") + ".");
      }
      catch (e) { setMessage(e && e.message ? e.message : String(e)); }
      setLoading(false);
    }
    async function openSelectedNow() {
      setLoading(true); setMessage("Opening selected channel...");
      try {
        var id = storage.selectedChannelId || storage.lastChannelId || rememberCurrentChannel();
        var result = await openChannelFast(id);
        setMessage("Open: " + result + ". Channel: " + (id || "none"));
      } catch (e) { setMessage(e && e.message ? e.message : String(e)); }
      setLoading(false);
    }
    ...
    function channelRow(channel) {
      var active = selectedChannelIds().indexOf(channel.id) !== -1 || (!storage.forceLoadChannels && !storage.selectedChannelId && storage.lastChannelId === channel.id);
      return React.createElement(Pressable, { key: channel.id, onPress: function () { chooseChannel(channel.id); }, style: { paddingVertical: 10, paddingHorizontal: 12, marginTop: 6, borderRadius: 8, borderWidth: 1, borderColor: active ? "#5865f2" : "#333", backgroundColor: active ? "#263168" : "#202020" } },
        React.createElement(Text, { numberOfLines: 1, style: { color: "white", fontWeight: active ? "900" : "700" } }, (storage.forceLoadChannels ? (active ? "[x] " : "[ ] ") : "") + "#" + channel.name),
    ...
    return React.createElement(ScrollView, { style: { padding: 16 } },
      React.createElement(Text, { style: { color: "white", fontSize: 24, fontWeight: "900" } }, "Channel Media Gallery 1.2.0"),
      React.createElement(View, { style: { marginTop: 10, padding: 12, borderRadius: 8, backgroundColor: "#202024", borderWidth: 1, borderColor: "#333" } },
        React.createElement(Text, { style: { color: "white", fontWeight: "800" } }, "Codex Config Tools"),
        React.createElement(Text, { style: { color: "#aaa", marginTop: 4, fontSize: 12 } }, "Quick access to scan, cache, filters, and force-load controls."),
        storage.forceLoadChannels ? React.createElement(Pressable, { disabled: loading, onPress: refreshGuild, style: { padding: 10, marginTop: 10, backgroundColor: "#323238", borderRadius: 8 } }, React.createElement(Text, { style: { color: "white", fontWeight: "700" } }, "Load Server Channels")) : null
      ),
      React.createElement(Text, { style: { color: "#aaa", marginTop: 8 } }, SERVER_CACHE_GUILD_NAME + " cache survives restarts. Other server caches last only for this session. Media stays hidden until you choose an action."),
      React.createElement(Text, { style: { color: "#777", marginTop: 10 } }, "Saved: " + storage.savedMedia.length + " | Showing: " + items.length + " | Server: " + (storage.selectedGuildId || storage.savedGuildId || getCurrentGuildId() || "none") + " | Channels: " + (selectedChannelIds().length || (storage.savedChannelId || storage.lastChannelId ? 1 : 0))),
      React.createElement(View, { style: { flexDirection: "row", marginTop: 12 } },
        React.createElement(Pressable, { onPress: function () { chooseLoadMode(false); }, style: { flex: 1, padding: 11, marginRight: 5, borderRadius: 8, backgroundColor: !storage.forceLoadChannels ? "#5865f2" : "#2b2b2b", alignItems: "center" } }, React.createElement(Text, { style: { color: "white", fontWeight: "800" } }, "Loaded")),
        React.createElement(Pressable, { onPress: function () { chooseLoadMode(true); }, style: { flex: 1, padding: 11, marginLeft: 5, borderRadius: 8, backgroundColor: storage.forceLoadChannels ? "#5865f2" : "#2b2b2b", alignItems: "center" } }, React.createElement(Text, { style: { color: "white", fontWeight: "800" } }, "Force"))
      ),
      React.createElement(Pressable, { disabled: loading, onPress: showLoadedCache, style: { padding: 12, marginTop: 10, backgroundColor: "#323238", borderRadius: 8, alignItems: "center" } }, React.createElement(Text, { style: { color: "white", fontWeight: "700" } }, "Show Loaded Cache")),
      storage.forceLoadChannels ? React.createElement(Pressable, { accessibilityRole: "switch", accessibilityState: { checked: storage.openBeforeForceLoad !== false }, onPress: function () { storage.openBeforeForceLoad = storage.openBeforeForceLoad === false; bump(); }, style: { padding: 10, marginTop: 8, borderRadius: 8, backgroundColor: storage.openBeforeForceLoad !== false ? "#5865f2" : "#2b2b2b" } }, React.createElement(Text, { style: { color: "white" } }, "Open channel before scan: " + (storage.openBeforeForceLoad !== false ? "ON" : "OFF"))) : null,
      storage.forceLoadChannels ? React.createElement(Pressable, { disabled: loading, onPress: openSelectedNow, style: { padding: 12, marginTop: 8, backgroundColor: "#323238", borderRadius: 8 } }, React.createElement(Text, { style: { color: "white" } }, "Open Selected Channel Now")) : null,
      ...
      React.createElement(Text, { style: { color: "#aaa", marginTop: 6 } }, storage.forceLoadChannels ? "Tap channels to select multiple. Run will scan every checked channel." : "Only loaded channels are listed. Enable Force load to choose a server and include channels without loaded messages."),
      storage.forceLoadChannels ? React.createElement(View, { style: { flexDirection: "row", flexWrap: "wrap", marginTop: 10 } },
        React.createElement(Pressable, { onPress: function () { selectShownChannels(false); }, style: { padding: 9, marginRight: 6, marginTop: 4, backgroundColor: "#323238", borderRadius: 8 } }, React.createElement(Text, { style: { color: "white" } }, "Select All Shown")),
        React.createElement(Pressable, { onPress: function () { selectShownChannels(true); }, style: { padding: 9, marginRight: 6, marginTop: 4, backgroundColor: "#323238", borderRadius: 8 } }, React.createElement(Text, { style: { color: "white" } }, "Select Loaded Only")),
        React.createElement(Pressable, { onPress: clearChannelSelection, style: { padding: 9, marginTop: 4, backgroundColor: "#5c2b2b", borderRadius: 8 } }, React.createElement(Text, { style: { color: "white" } }, "Clear"))
      ) : null,
  ...
  function onUnload() { if (timer) clearInterval(timer); timer = null; }
