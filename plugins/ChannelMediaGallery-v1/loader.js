...
var error=null;
var last='Ready';
var corePath='ChannelMediaGallery-v1';
var title='Channel Media Gallery Config 1.1.28';
...
function urlList(){var t='v=1.1.28&t='+Date.now();return ['https://raw.githubusercontent.com/Artkos078/kettu-plugins/main/plugins/ChannelMediaGallery-v1/index.js?'+t];}
...
async function load(force){if(loading)return;if(runtime&&!force)return;loading=true;error=null;say('Loading gallery...');var urls=urlList(),errs=[];try{for(var i=0;i<urls.length;i++){try{var src=await fetchText(urls[i]);if(!src||src.indexOf('Channel Media Gallery')<0)throw new Error('Wrong file');var CV={metro:V.metro,ui:V.ui,utils:V.utils,patcher:V.patcher,storage:coreStorage,plugin:{id:'',storage:coreStorage}};runtime=(0,eval)('(function(vendetta){return '+patch(src)+';})')(CV);if(runtime&&runtime.default)runtime=runtime.default;if(runtime&&runtime.onLoad)runtime.onLoad();else if(runtime&&runtime.start)runtime.start();say('Channel Media Gallery loaded');return;}catch(e){errs.push(urls[i]+' -> '+text(e));}}throw new Error(errs.join('\n'));}catch(e2){error=e2;say('Gallery load failed');}finally{loading=false;}}
