(function(){
  ...
  function save(){
    ...
    }; } catch(e) { console.log('[PrivateChats] save failed', e); }
  }
  ...
  function stores(){
    return {
      ...
      selectedChannel: findByProps('getChannelId','getVoiceChannelId') || findByProps('getLastSelectedChannelId') || findByProps('getChannelId'),
      selectedGuild: findByProps('getGuildId','getLastSelectedGuildId') || findByProps('getGuildId'),
  ...
  function after(obj,name,fn){
    ...
    try { var u=patcher.after(name,obj,function(args,ret){ return fn(ret,args); }); if(typeof u==='function') unpatches.push(u); } catch(e){}
  }
  ...
  function addCurrentServer(){ var c=context(); if(!c.guildId){toast('Open a server first');return;} if(state.hiddenGuildIds.indexOf(c.guildId)===-1)state.hiddenGuildIds.push(c.guildId); save(); unlocked=false; refresh(); toast('Protected '+((c.guild&&c.guild.name)||'current server')); }
  function addCurrentDM(){ var c=context(); if(!c.channelId){toast('Open a DM first');return;} if(c.guildId){toast('Open a DM, not a server channel');return;} if(state.hiddenChannelIds.indexOf(c.channelId)===-1)state.hiddenChannelIds.push(c.channelId); save(); unlocked=false; refresh(); toast('Protected current DM'); }
  ...
  function Settings(){
    ...
    var View=RN.View, Text=RN.Text, TextInput=RN.TextInput, Pressable=RN.Pressable, Switch=RN.Switch, ScrollView=RN.ScrollView;
    var a=React.useState(''), code=a[0], setCode=a[1];
    var b=React.useState(String(state.passcode)), newPass=b[0], setNewPass=b[1];
    ...
    function rerender(){setTick(tick+1);}
    ...
    function row(id,type){ return React.createElement(View,{key:type+id,style:{flexDirection:'row',justifyContent:'space-between',paddingVertical:10}},React.createElement(Text,{style:{color:'white',flex:1}},id),React.createElement(Pressable,{onPress:function(){ if(type==='g')state.hiddenGuildIds=state.hiddenGuildIds.filter(function(x){return x!==id;}); else state.hiddenChannelIds=state.hiddenChannelIds.filter(function(x){return x!==id;}); save();refresh();rerender();}},React.createElement(Text,{style:{color:'#ff6b6b',fontWeight:'700'}},'Remove'))); }
    return React.createElement(ScrollView,{style:{padding:16}},
      ...
      btn('Protect Current Server',function(){addCurrentServer();rerender();}),
      btn('Protect Current DM',function(){addCurrentDM();rerender();}),
      React.createElement(Text,{style:{fontWeight:'700',color:'white',marginTop:18}},'Protected servers'),
      state.hiddenGuildIds.length?state.hiddenGuildIds.map(function(id){return row(id,'g');}):React.createElement(Text,{style:{color:'#888'}},'None'),
      React.createElement(Text,{style:{fontWeight:'700',color:'white',marginTop:18}},'Protected DMs'),
      state.hiddenChannelIds.length?state.hiddenChannelIds.map(function(id){return row(id,'c');}):React.createElement(Text,{style:{color:'#888'}},'None'),
      ...
      btn('Save Passcode',function(){if(String(newPass).trim()){state.passcode=String(newPass).trim();save();unlocked=false;refresh();setStatus('LOCKED');toast('Passcode saved');}}),
      React.createElement(View,{style:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginTop:18}},React.createElement(Text,{style:{color:'white',flex:1}},'Hide media while locked'),React.createElement(Switch,{value:!!state.hideMedia,onValueChange:function(v){state.hideMedia=!!v;save();rerender();}}))
    );
  }

  function start(){ unlocked=false; save(); patchStores(); toast('PrivateChats loaded'); }
  function stop(){ unlocked=false; while(unpatches.length){ try{unpatches.pop()();}catch(e){} } }

  return { onLoad:start, onUnload:stop, settings:Settings };
})()