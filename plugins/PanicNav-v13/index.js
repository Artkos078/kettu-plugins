...
function patchGlobalLayer(){
  ...
  try{
    ...
    var toastMemo=MainShared&&MainShared.ToastContainer;
    if(toastMemo&&typeof toastMemo.type==='function'){
      var u=patcher.after('type',toastMemo,function(args,ret){
        if(pstore.overlayEnabled===false)return ret;
        return React.createElement(React.Fragment,null,ret,React.createElement(FloatingControls,{key:'panicnav-floating-v13'}));
      });
      if(typeof u==='function')unpatches.push(u);
      overlayHook='ToastContainer.type';
      return true;
    }
  }catch(e){try{console.log('[PanicNav] global hook error',e);}catch(_) {}}
  return false;
}
...
function Settings(){
  ...
  return React.createElement(ScrollView,{style:{padding:16}},
    ...
    React.createElement(Text,{style:{color:'#777',marginBottom:14}},'v1.3 mounts beside Discord\'s global ToastContainer. Drag the ≡ handle to move it.'),
...
function onLoad(){var ok=patchGlobalLayer();toast(ok?'PanicNav v1.3 global overlay hooked':'PanicNav v1.3: ToastContainer hook missing');}
